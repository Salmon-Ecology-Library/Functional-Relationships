...
td <- read.csv("data/beamer_pess_1999_v2.csv")
plot_name <- "beamer_pess_1999_v2"

# X-variable
x <- td$x
xlb <- "Incubation flows (cfs)" # label used in plots
...
legend("topright", legend = c("Extracted data", "Fitted equation"), col = c(1, 4), lwd = c(NA, 2), pch = c(19, NA), bty = 'n')
dev.off()
